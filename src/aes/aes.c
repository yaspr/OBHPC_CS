...
//
#define KEY_16       16
#define KEY_EXP_16  176

#define KEY_24       24
#define KEY_EXP_24  208

#define KEY_32       32
#define KEY_EXP_32  240
...
static const byte sbox[MAX_256] = {
...
//
static const byte rsbox[MAX_256] = {
...
//
static const byte mul2[MAX_256] = {
...
//
static const byte mul3[MAX_256] = { 
...
//
static const byte mul9[MAX_256] = {
...
//
static const byte mul11[MAX_256] = {
...
//
static const byte mul13[MAX_256] = {
...
//
static const byte mul14[MAX_256] = {
...
//
static const byte rcon[MAX_256] = {
...
//
void _expand_key_(byte *in, byte i)
{
  //
  u32 *q = (u32 *)in;
  *q = (*q >> 8) | ((*q & 0xff) << 24);
...
//
void rev_shift_rows(byte *state)
{
  byte tmp[BLOCK_SIZE];
...
//
void rev_mix_columns(byte *state)
{
  byte tmp[BLOCK_SIZE];
...
//
void add_round_key(byte *state, byte *key)
{
  for (byte i = 0; i < BLOCK_SIZE; i++)
    state[i] ^= key[i];
}
...
//
void AES_encrypt(byte *in, byte *keys, byte *out)
{
  byte state[BLOCK_SIZE];
  u32 nb_rounds = NB_ROUNDS;
  ...
  //Rounds
  for (byte i = 0; i < nb_rounds; i++)
    {
      sub_bytes(state);
      shift_rows(state);
      mix_columns(state);
      add_round_key(state, keys + (BLOCK_SIZE * (i + 1))); //each round has a key
    }

  //Final round
  sub_bytes(state);
  shift_rows(state);
  add_round_key(state, keys + (KEY_EXP_16 - KEY_16)); //Last key
...
//
void AES_decrypt(byte *in, byte *keys, byte *out)
{
  byte state[BLOCK_SIZE];
  u32 nb_rounds = NB_ROUNDS;
  ...
  add_round_key(state, keys + (KEY_EXP_16 - KEY_16));

  for (byte i = nb_rounds; i > 0; i--)
    {
      rev_shift_rows(state);
      rev_sub_bytes(state);
      add_round_key(state, keys + (BLOCK_SIZE * i));
...
//
byte encrypt_file(byte *ifname, byte *key, byte *ofname)
{
  FILE *ifd = fopen(ifname, "rb");
  ...
  if (ifd)
	...
	return 2;
      ...
      byte xkeys[KEY_EXP_16];
      
      //Key expansion
      expand_key(key, xkeys);

      //
      while ((len = fread(in, sizeof(byte), BUFFER_SIZE, ifd)))
	{
	  u64 plen = len;
	  
	  //Closest value divisible by 16 
	  if (plen & 15)
	    plen = ((plen >> 4) + 1) << 4;
	  
	  for (u64 i = 0; i < plen; i += BLOCK_SIZE)
	    AES_encrypt(in + i, xkeys, out + i);
	  
	  fwrite(out, sizeof(byte), plen, ofd);
	}
...
//
byte decrypt_file(byte *ifname, byte *key, byte *ofname)
{
  FILE *ifd = fopen(ifname, "rb");
  FILE *ofd = fopen(ofname, "wb");
  ...
  if (ifd)
	...
	return 2;
      ...
      while ((len = fread(in, sizeof(byte), BUFFER_SIZE, ifd)))
	{
	  for (u64 i = 0; i < len; i += BLOCK_SIZE)
	    AES_decrypt(in + i, xkeys, out + i);
	  
	  fwrite(out, sizeof(byte), len, ofd);
	}
...
{
  if (argc < 5)
    return printf("Usage: %s [-e | -d] [key] [input file] [output file]\n", argv[0]), -1;
  ...
  if (mode == 'e')
    {
      byte r = encrypt_file(argv[3], argv[2], argv[4]);
    ...
    if (mode == 'd')
      {
	byte r = decrypt_file(argv[3], argv[2], argv[4]);
